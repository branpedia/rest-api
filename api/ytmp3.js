...
export default async function handler(req, res) {
  ...
  const { url, format = 'audio', retry = 0, backup = 'false' } = req.query;
  ...
  try {
    ...
    // If backup parameter is true, use backup server directly
    if (backup === 'true') {
      console.log('Using backup server method...');
      ...
      try {
        ...
        try {
          ...
          // If second method fails, try third method (cloudscraper - backup server)
          try {
            console.log('Trying third method (backup server - cloudscraper)...');
            result = await convertWithCloudscraper(url);
          } catch (error3) {
    ...
    return res.status(200).json({
      success: true,
      data: result,
      method: backup === 'true' ? 'backup-server' : 'primary-server'
    });

  } catch (error) {
    console.error('Error:', error.message);
    
    // Retry logic with backup server as fallback
    if (retry < 3) {
      // Wait for 1 second before retrying
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // On last retry, try backup server
      if (parseInt(retry) + 1 >= 2) {
        return handler({ 
          ...req, 
          query: { 
            ...req.query, 
...
// Third download method (cloudscraper) - BACKUP SERVER
